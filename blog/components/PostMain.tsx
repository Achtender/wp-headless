...
const WP_BASEHREF = Deno.env.get("WP_BASEHREF")!;

export function PostMain({ post }: { post: WpPost }) {
  return (
    <main class="mx-auto max-w-screen-lg pb-10 flex-1">
      {/* <h1
      ...
      <section class="mx-auto text-lg">
        <div
          class="post-content"
          dangerouslySetInnerHTML={{ __html: post.content.rendered.replaceAll(`href="${WP_BASEHREF}`, 'href="/') }}
        />
        <br class="clear-both" />
        {post.type === "post" && (
          <>
...
//       <h2 class="text-2xl">
//         Responses to{" "}
//         "<span dangerouslySetInnerHTML={{ __html: post.title.rendered.replaceAll(WP_BASEHREF, '/') }} />"
//       </h2>
//       {replies.map((reply) => (
...
//           <div
//             class="mt-4 px-2"
//             dangerouslySetInnerHTML={{ __html: reply.content?.rendered?.replaceAll(WP_BASEHREF, '/')! }}
//           />
//         </div>
//       ))}
//     </section>
